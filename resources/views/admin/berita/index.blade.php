                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($allBerita as $item)
                                <tr>
                                    <th scope="row">1</th>
                                    <td class="fw-bold">{{ Str::limit($item->judul, 10) }}</td>
                                    <td>{!! Str::words($item->isi, 20) !!}</td>
                                    <td>{{ $item->kategori->nama_kategori }}</td>
                                    <td>{{ $item->user->name }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{route('berita.edit',$item->id)}}" class="btn btn-success btn-sm">Edit</a>
                                            <form onsubmit="return confirm('sure to delete this data')"
                                                action="{{ route('berita.delete', $item->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-sm btn-danger mb-0 ms-1">Delete</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
